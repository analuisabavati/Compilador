...
public class ArquivoUtil {
	...
	private final static String pathArquivo = "C:\\Users\\leona\\Desktop\\teste5.txt";
	protected final static String pathArquivoTratado = "C:\\Users\\leona\\Desktop\\arquivoTratado.txt";
	...
	public static Scanner abreArquivoFonte(String path) throws Exception, FileNotFoundException {
		if (path == null) {
	...
	public static String trataComentarioChaves(String linhaArquivo) throws Exception {
		...
		while (indexCaractereLinha < linhaArquivo.length()) {
			...
			if(caractereLinha == '}') {
				throw new Exception ("Erro na linha " + numeroLinha + ". Formato de comentario invalido!");
			}
				...
				if (caractereLinha == '{') {
					while (caractereLinha != '}') {
						...
						if (indexCaractereLinha == linhaArquivo.length()) {
							throw new Exception("Erro! Comentario nao fechado na linha " + numeroLinha);
						}
						caractereLinha = linhaArquivo.charAt(indexCaractereLinha);
					}
				}
	...
	public static String trataComentarioBarraAsterisco(String linhaArquivo) throws Exception {
		int indexCaractereLinha = 0;
		char caractereLinha;
		...
		while(indexCaractereLinha < linhaArquivo.length()) {
			caractereLinha = linhaArquivo.charAt(indexCaractereLinha);
			if((caractereLinha == '*') && (linhaArquivo.charAt(indexCaractereLinha + 1) == '/')) {
				throw new Exception ("Erro na linha " + numeroLinha + ". Formato de comentario invalido!");
			}
			while((caractereLinha == '/') && (proximoCaractereLinha == '*')) {
				if((indexCaractereLinha + 2) >= linhaArquivo.length()) {
					throw new Exception ("Erro na linha " + numeroLinha + ". Comentario aberto e nao fechado!");
				}
			...
			if((indexCaractereLinha + 1) >= linhaArquivo.length()) {
				novaLinha = novaLinha + linhaArquivo.charAt(indexCaractereLinha);
				break;
			}
			else {
				proximoCaractereLinha = linhaArquivo.charAt(indexCaractereLinha + 1);
